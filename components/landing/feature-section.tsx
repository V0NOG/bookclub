        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, i) => (
            <div key={feature.title} className="rounded-xl border border-border bg-card p-6 hover:border-border/80 transition-colors">
              <div className={`inline-flex h-10 w-10 items-center justify-center rounded-lg mb-4 ${i % 2 === 0 ? "bg-primary/10" : "bg-secondary/10"}`}>
                <feature.icon className={`h-5 w-5 ${i % 2 === 0 ? "text-primary" : "text-secondary"}`} />
              </div>
