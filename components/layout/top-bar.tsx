  ...
  return (
    <header className="sticky top-0 z-30 h-16 border-b border-border bg-background/95 backdrop-blur flex items-center px-6 gap-4">
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <Link href="/profile">
          <div className="h-8 w-8 rounded-full bg-emerald-500/20 ring-2 ring-transparent hover:ring-emerald-500 transition-all flex items-center justify-center cursor-pointer overflow-hidden">
            {session?.user?.image ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={session.user.image} alt="" className="h-8 w-8 rounded-full object-cover" />
            ) : (
              <span className="text-xs text-emerald-400 font-semibold">{initial}</span>
            )}
          </div>
        </Link>
      </div>
